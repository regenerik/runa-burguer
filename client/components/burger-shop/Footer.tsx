  ...
  return (
    <footer className="py-12 bg-card/80 border-t border-border/50">
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-8 right-8 z-50 bg-[#25D366] hover:bg-[#20ba59] text-white px-6 py-3 rounded-full font-bold flex items-center gap-2 shadow-2xl shadow-green-900/40 transition-transform hover:scale-110 md:hidden relative"
      >
        Pedir
        <MessageCircle size={20} fill="currentColor" />
        {cartCount > 0 && (
          <span className="absolute -top-2 -right-2 bg-primary text-primary-foreground text-[10px] font-bold w-6 h-6 rounded-full flex items-center justify-center border-2 border-background shadow-lg animate-in zoom-in duration-300">
            {cartCount}
          </span>
        )}
      </a>
